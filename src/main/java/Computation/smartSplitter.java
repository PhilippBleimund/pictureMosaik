...
public class smartSplitter {
	...
	public splitObj splitImage(BufferedImage bi, int splits, int multiplier) {
		int xBi = bi.getWidth();
		int yBi = bi.getHeight();
		
		int numberXSplits = xBi / splits;
		int numberYSplits = yBi / splits;
		
		int restX = xBi - (numberXSplits * splits);
		int restY = yBi - (numberYSplits * splits);
		
		//The rest pixels are now distributed along the other Splits
		int[] valuesX = distributeValues(splits, numberXSplits, restX);
		int[] valuesY = distributeValues(splits, numberYSplits, restY);
