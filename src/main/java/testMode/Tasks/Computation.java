...
public class Computation implements Runnable {
  ...
  private void testComputation(TestConfig AllConfig) {
    ...
    Color[][] averageColorSections =
        new Color[config.getSectionsX()][config.getSectionsY()];
    Random rnd = new Random();
    for (int j = 0; j < averageColorSections.length; j++) {
      for (int k = 0; k < averageColorSections[0].length; k++) {
        averageColorSections[j][k] =
    ...
    for (int i = 0; i < AllConfig.getRepeat(); i++) {
      ...
      File[][] choosen =
          compare.compare(averageColorSections, Obj, config.getMaxUsage());
  ...
  private void Log(long time, int ite) {
    if (ite % 50 == 0) {
      TestModeManager.getInstance().Log(
          "Iteration: " + ite + " time: " + (time) + "ns", false);
      TestModeManager.getInstance().Log(
          "Iteration: " + ite + " time: " + helper.nanoToString(time), false);
    }
