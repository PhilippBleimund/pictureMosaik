...
public class ImagesTest implements Runnable {
  ...
  private long testAverageColor(ImagesConfig config, File[] images) {
    ...
    Color[] averageColorFiles = colorCalculator.computeAverageColorFiles(
        images, calculateAverage.ScalrToThis(config.getMethod()));
  ...
  private long testDownrender(ImagesConfig config, File[] images) {
    ...
    Dimension d =
        new Dimension((int)config.getNewSize().getWidth() * images.length,
                      (int)config.getNewSize().getHeight());
    splitObj splitImage =
        imageSplitter.splitImage(null, d, images.length, 0, 1);

    downrenderFiles downrenderChoosen = new downrenderFiles();
    long timerDownrender0 = System.nanoTime();
    downrenderChoosen.downrenderFilesAndSave(imagesChoosen, splitImage,
                                             config.getMethod());
  ...
  private void Log(long time, int threshold) {
    if (this.TestId % threshold == 0) {
      TestModeManager.getInstance().Log(
          "Iteration: " + TestId + " time: " + (time) + "ns", false);
      TestModeManager.getInstance().Log(
          "Iteration: " + TestId + " time: " + helper.nanoToString(time),
          false);
      TestModeManager.getInstance().Log("", true);
      TestModeManager.getInstance().Log("-+-+-+-", true);
      TestModeManager.getInstance().Log("", true);
    }
    TestModeManager.getInstance().LogToJSON(JSONObj.TimeValuesKey,
                                            Long.toString(time));
    Runtime runtime = Runtime.getRuntime();
    long memory = runtime.totalMemory() - runtime.freeMemory();
    long maxMemory = runtime.maxMemory() - runtime.totalMemory();
    TestModeManager.getInstance().LogToJSON(JSONObj.RamValuesKey,
