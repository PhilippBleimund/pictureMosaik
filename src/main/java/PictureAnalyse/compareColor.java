...
public class compareColor {
	...
	public File[][] compare(Color[][] colorArr, DatabaseObj[] Databases, int maxTimes){
		...
		class compareDatabases implements Runnable{
			...
			public void run() {
				...
				for(int l=0;l<Databases.length;l++) {
					int value = colorArr[x][y].getRGB() - Databases[l].filesAndColors[Databases[l].compability[x][y]].color.getRGB();
	...
	public static int getDatabaseBestColor(Color color, DatabaseObj Database, int maxRepetition) {
		...
		if(insertionPoint > 0) {
			insertionPoint = (Database.filesAndColors[insertionPoint].color.getRGB() - color.getRGB()) < (color.getRGB() - Database.filesAndColors[insertionPoint -1].color.getRGB()) ?
		...
		int rigthDistance;
		if(leftBest > -1) {
			leftDistance = color.getRGB() - Database.filesAndColors[leftBest].color.getRGB();
		...
		if(rigthBest > -1) {
			rigthDistance = color.getRGB() - Database.filesAndColors[rigthBest].color.getRGB();
